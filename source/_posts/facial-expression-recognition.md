...
## 数据准备
数据集和预训练模型均已经上传到百度网盘，[链接](https://pan.baidu.com/s/1LFu52XTMBdsTSQjMIPYWnw)给出，提取密码为2pmd。下载后将`model
.zip`移动到根目录下的`models`文件夹下并解压得到一个`*.h5`的模型参数文件，将`data.zip`移动到根目录下的`dataset`文件夹下并解压得到包含多个数据集压缩文件，均解压即可得到包含图像的数据集（**其中rar后缀的为原始jaffe数据集，这里建议使用我处理好的**）。
...
![](https://i.loli.net/2020/08/25/TtcFkPSm3vgZbME.png)

![](https://i.loli.net/2020/08/25/faKs6yzbLciUvxm.png)
...
```
![](https://i.loli.net/2020/08/25/urIbfVFaGiz5EdW.png)
...
python src/gui.py
```
![](https://i.loli.net/2020/08/25/f9VxPwk1dGJ8NKC.png)

上图的GUI反馈的同时，会对图片上每个人脸进行检测并表情识别，处理后如下图。

![](https://i.loli.net/2020/08/25/BmxSpOt1X4RVEUe.png)
...
![](https://i.loli.net/2020/08/25/sCyxpwNifDJLEd9.gif)

## 补充说明
具体项目代码、数据集、模型已经开源于[我的Github](https://github.com/luanshiyinyang/FacialExpressionRecognition.git)，欢迎Star或者Fork。
